	...
	while (true) {
		if (GetAsyncKeyState(VK_NUMPAD0) & 1) {
			...
			while (true) {
				...
				if (mainBase.getCurrentAnimIdForOtherPlayer(0) == 67011) {
					Log("player respawning, entity id last hit by-");
					int lastHitEntityID = mainFunctions.getEntityID(lastEntityHitBy);
					Log(lastHitEntityID);
					mainFunctions.applyEffect(100, lastHitEntityID);//should apply full hp restoration
					//at this point emevd can detect that a given entity has a specific speffect and link this to event scripting
				}
